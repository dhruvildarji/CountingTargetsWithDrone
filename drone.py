...
import socket         # socket is used for communication and acknowledgement betweeen drone and raspberry pi
import time         #   Time is needed in this project
import math         
from dronekit import connect, VehicleMode, LocationGlobalRelative   # dronekit is used for autonomous flights of the drone
from pymavlink import mavwp     # Controller
import numpy as np      # numpy 
import struct       # for data packets

HOST = '10.1.1.130'    # The remote host   --> Raspberry PI
PORT = 50007              # The same port as used by the server
#meters	4 f = 1.2192, 3 = 0.9144, 2 = 0.6096,1 = 0.3048,10 inch = 0.254, 8 inch = 0.2032, 6 inch = 0.1524, 4 inch = 0.1016
DISTANCE = 15           #Meters --> drone's flight length 
STEP_SIZE = 3           #Meters --> size of one image captured by the drone
OVERLAP = 1             # Meters --> Overlap between two images
GPS_ERROR = 1           # Meters --> GPS Error is considered
CONST_HORI = 1.1438         # Calculated from Horizontal Field of View --> Description in Read Me file
...
while 1:
	...
	point1 = LocationGlobalRelative(new_lat, new_lon, ALT)
	vehicle.simple_goto(point1)     #give command to drone so it will go in that direction
	time.sleep(5)                   #give it 5 seconds to reach there
	yaw = vehicle.attitude.yaw      #get yaw 
	yaw = math.degrees(yaw)         #convert it in degrees
	alpha = math.degrees(np.arctan2(lat,lon))       #part of the algorithm, explained in read me file
	if alpha < 0:                   
		alpha = alpha + 360
	alpha = alpha + 90
	direction = yaw + alpha
	...
	data = s.recv(1024)                 # wait for the acknowledgement
	if data == 'pic_taken':                 
		print "confirmation : pic has taken"    
		if dist >= DISTANCE:        #The loop will be repeated untill the drone covered the desired distance.
			print "Returning to Land"               
			vehicle.mode = VehicleMode("LAND")      #drone will land
