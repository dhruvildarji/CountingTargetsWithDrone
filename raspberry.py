import socket                           # for connecion between drone and raspberry pi
from picamera import PiCamera           # for camera of raspberry pi --> capture the images
import os                               
import time                             #time
...
class TargetCounter(object):    #Class for Algorithm
	...
	def count(self,num_Image):              #Brain of the code 
		if num_Image == 1:                              #if it is first image then count the last target of the previous image out of the image
			self.prev_last_target_x = -1            #X coordinate of the last target of the previous image
			self.prev_last_target_y = -1            #Y coordinate of the last target of the previous image
		print "Image",num_Image
		image_Path = self.Folder+'Image'+str(num_Image)+'.jpg'          #Imagepath
		image = cv2.imread(image_Path)                                  #read the image
		Current_Contours = self.Find_Contours(image)                    #find number of targets in the image
		if len(Current_Contours) > 0:                                   # if there is any target in the image apply the algorithm
			last_Target_x,last_Target_y = self.Find_Last_Target(Current_Contours)   #find the last target of the image
			if self.prev_last_target_x < 0:                                             #if last target of the previous image is not in the image, then count the total available targets in the image.
				self.numTargets = self.numTargets + len(Current_Contours)               #count the targets
		 	else:
				index, target_center_x, target_center_y = self.Find_Prev_Last_Target(Current_Contours)  #find the last target of the previous image in the new image
	...
	def Find_Contours(self,img):
		THRESHOLD_LOW = (0,100,100)#(0,50,50)                           # Threshold Low of Red Color
		...
		img_binary = cv2.dilate(img_binary, None, iterations = 15)	        #dilate the image
	...
	def Find_Last_Target(self,contours):                    # find the last target of the current image
		...
		if len(contours) > 0:	# if at least one object is detected
			for i in range(len(contours)):
				...
				#print "New Targets " + str(i+1) + " @ (" + str(x) + ", " + str(y) + ")"	
				...
				M = cv2.moments(contours[i])		# find moment of each object 
				if M["m00"] > 0:		#this method is only for rounding outside the objects
					center = (int(M["m10"]/M["m00"]), int(M["m01"]/M["m00"])) 		
				else:
					print "nothing"
			a = distance.index(min(distance))		# find the last object of the image
			#print "Last point in first image",x_list[a],y_list[a]	#print it
			last_point_x = x_list[a]
			last_point_y = y_list[a]
			return last_point_x,last_point_y
	def Find_Prev_Last_Target(self,contours):               #find the last target of the previous image in the current image
		point_new = []
		x_list1 = []
		y_list1 = []
		new_vector_x = []
		...
		center = None
		radius = 0
		if len(contours) > 0:				#same procedure will repeat for figure 2
			for i in range(len(contours)):
				...
				x_list1.append(x)
				y_list1.append(y) 
				new_vector_x.append(x-self.prev_last_target_x) 		#find the vector from all new objects to last object of the previous image
				new_vector_y.append(y-self.prev_last_target_y) 		
				unit_vector = Vector(new_vector_x[i],new_vector_y[i],0)
				...
				point_new.append(unit_vector)		#save all the vectores in this new list
				M = cv2.moments(contours[i])					 
		deviation = []
		for j in range(len(point_new)):
			d = self.direction.angle(point_new[j])          #The angle between the direction vector and from the ghost to all the real targets
		...
		x = x_list1[a]
		y = y_list1[a]
		#print "Last point of the previous image",x_list1[a],y_list1[a],a	#print that point 
	...
	def Count_New_Targets(self,index,target_center_x,target_center_y):      #distinguish between new and old targets
		...
		for i in range(len(target_center_x)):					
			...
			a = self.direction.dot(unit_vector)	# if dot product of the direction and new vectors is negative , then it is the old objects in the new image
...
while 1:
	frame = frame + 1                               #increasing the number of the image
	data = conn.recv(1024)                          #receiving the data
	if data == 'stop':
		conn.close()
		break
	...
	Artesia_Counter.set_step_size(step_size)        # step the step size
